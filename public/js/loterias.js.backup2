...
// ============================================
// FUNÇÃO AUXILIAR PARA FORMATAR RESULTADO LOTECA
// ============================================
function formatarResultadoLoteca(valor) {
    if (valor === 0) return "⚖️ Empate";
    if (valor === 1) return "🏠 Coluna 1";
    if (valor === 2) return "✈️ Coluna 2";
    return "?";
}

// ============================================
// PROCESSAR CSV (COM CACHE E SUPORTE A LOTECA)
// ============================================
function processarCSV(loteria, texto, nome) {
    ...
    // Força separador ; para Loteca, senão detecta automaticamente
    let sep = ';';
    if (loteria !== 'loteca') {
        sep = linhas[0].includes(';') ? ';' : ',';
    }
    ...
    for (let i = 0; i < linhas.length; i++) {
        ...
        for (let j = dataIndex + 1; j < colunas.length; j++) {
            ...
            // 🆕 TRATAMENTO ESPECIAL PARA LOTECA
            if (loteria === 'loteca') {
                if (valor === 'Coluna 1') {
                    numeros.push(1);      // Vitória do mandante
                } else if (valor === 'Coluna do meio') {
                    numeros.push(0);      // Empate
                } else if (valor === 'Coluna 2') {
                    numeros.push(2);      // Vitória do visitante
                }
                continue;  // Pula o resto do processamento numérico
            }
            
            // Processamento normal para outras loterias
        ...
        // Validação para Loteca: precisa ter exatamente 14 resultados
        if (loteria === 'loteca') {
            ...
            // Validação normal para outras loterias
            if (numeros.length >= config.numeros) {
...
window.formatarResultadoLoteca = formatarResultadoLoteca;
